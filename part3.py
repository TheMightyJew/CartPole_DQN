...
class DQNAgent:
    ...
    def train(self, terminal_state, step):
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            if not done:
                max_future_q = np.argmax(current_qs_list[index])
                q_target_val = future_qs_list[index][max_future_q]
                new_q = reward + DISCOUNT * q_target_val
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        ...
        if curr_moving_avg >= 475:
            ...
            if curr_moving_avg > max_until_now + 1:
                ...
                agent.model.save(
                    f'models/{MODEL_NAME}__episodes__{EPISODES}__avg__{curr_moving_avg}__min__{int(time.time())}.model')
